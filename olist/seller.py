...
class Seller:
    ...
    def get_review_score(self):
        ...
        """
        # get data
        orderinfo = self.order.get_review_score()
        sellerinfo = self.data['order_items']\
            .loc[:,['order_id', 'seller_id']]
        df = pd.merge(sellerinfo, orderinfo, on='order_id')

        return df.groupby('seller_id', as_index=False)\
            .agg({
    ...
    def get_review_count(self):
        """
        Returns a DataFrame with:
        'seller_id', 'n_one_star', 'n_two_star', 'n_three_star'
        """
        # get data
        orderinfo = self.order.get_review_score()
        sellerinfo = self.data['order_items']\
        ...
        # dim_is_two_star
        df.loc[:,'dim_is_two_star'] = \
            df['review_score'].apply(dim_two_star)
        # dim_is_three_star
        df.loc[:,'dim_is_three_star'] = \
            df['review_score'].apply(dim_three_star)
        ...
        return df.groupby('seller_id', as_index=False)\
            ...
            'dim_is_three_star': 'sum'
                        ...
                        'dim_is_one_star': 'n_one_star',
                        'dim_is_two_star': 'n_two_star',
                        'dim_is_three_star': 'n_three_star'
                        })
    ...
    def get_training_data(self):
        ...
        'quantity_per_order', 'sales', 'n_one_star', 'n_two_star', 'n_three_star']
        """
        ...
        training_set =\
            self.get_seller_features()\
                ...
                self.get_review_count(), on='seller_id'
               ).merge(
